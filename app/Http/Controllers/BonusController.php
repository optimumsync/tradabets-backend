...
class BonusController extends controller
{
    ...
    public function index(Request $request)
    {
        ...
        \Log::info('Transaction count:', ['count' => count($transaction)]);
    ...
    public function bonusTransactionList(Request $request)
    {
        ...
        if($request->form){
            $transaction=Transaction::where([['user_id',$user->id],['status','bonus']])
                ->whereBetween('created_at',array($request->form['date_from'],$request->form['date_to']))->get()->all();
        }
        else
        {
        ...
        return view('bonus.bonus-transaction-list-index',$view_data);
    }
}
